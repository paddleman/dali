defmodule DaliWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: DaliWeb.Gettext
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(DaliWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(DaliWeb.Gettext, "errors", msg, opts)
    end
  end
  ...
  @doc """
  Renders a file upload input for images.

  ## Examples

      <.image_upload upload={@uploads.images} />
      <.image_upload upload={@uploads.images} label="Upload Photos" />
  """
  attr :upload, :any, required: true, doc: "Phoenix LiveView upload config"
  attr :label, :string, default: "Upload Images"
  attr :help_text, :string, default: "PNG, JPG, GIF up to 10MB"
